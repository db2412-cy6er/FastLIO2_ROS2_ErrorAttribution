#include "global_relocalization/global_relocalization.hpp"
...
GlobalRelocalization::GlobalRelocalization() : Node("global_relocalization")
{
  ...
  pcd_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
    "registered_scan", 10, 
    std::bind(&GlobalRelocalization::registeredPcdCallback, this, std::placeholders::_1));

  initial_pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
    "initialpose", 10, 
    std::bind(&GlobalRelocalization::initialPoseCallback, this, std::placeholders::_1));

  register_timer_ = this->create_wall_timer(
    std::chrono::milliseconds(500),  // 2 Hz
    std::bind(&GlobalRelocalization::performRegistration, this));

  transform_timer_ = this->create_wall_timer(
    std::chrono::milliseconds(50),  // 20 Hz
    std::bind(&GlobalRelocalization::publishTransform, this));
      
  RCLCPP_INFO(this->get_logger(), MAG "GlobalRelocalization START." RST);
}

GlobalRelocalization::~GlobalRelocalization()
{
  RCLCPP_INFO(this->get_logger(), MAG "GlobalRelocalization OVER." RST);
}

void GlobalRelocalization::registeredPcdCallback
(const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)
{
  ...
  // 累积几帧点云，提高点云密度，增强匹配效果
  *accumulated_cloud_ += *scan;
}

void GlobalRelocalization::performRegistration()
{
  ...
  // 设置 GICP 参数
  register_->reduction.num_threads = num_threads_;  // 线程数
  register_->rejector.max_dist_sq = max_dist_sq_;   // 最大距离平方
  register_->optimizer.max_iterations = 10;         // 最大迭代次数
...
void GlobalRelocalization::loadGlobalMap(const std::string & file_name)
{
...
void GlobalRelocalization::publishTransform()
{
...
void GlobalRelocalization::initialPoseCallback

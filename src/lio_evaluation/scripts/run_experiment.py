...
def _fastlio_directional_args(exp_dir, directional=False, beta_t=None, beta_r=None,
                              ht_accum_enable=False, ht_accum_alpha=None):
    """fastlio launch 附加 P4 directional 参数 (唯一参数源 = launch, 依赖 degeneracy 门控)。

    --directional 时返回 directional_enable:=true (+ 可选 beta/累积覆盖)。
    P4 评审 R29: 与 --adaptive 正式实验互斥 (这里不强硬拦截, 由实验者纪律保证;
    C++ 侧会打 WARN 提示)。
    """
    if not directional or not _degen_enabled(exp_dir):
        return []
    args = ["directional_enable:=true"]
    if beta_t is not None:
        args.append(f"directional_beta_t:={beta_t}")
    if beta_r is not None:
        args.append(f"directional_beta_r:={beta_r}")
    if ht_accum_enable:
        args.append("directional_ht_accum_enable:=true")
...
def write_resolved_snapshot(exp_dir, drive=None, fault_cfg=None, adaptive=None,
    ...
    if directional:
        resolved["directional"]["enable"] = bool(directional.get("enable", False))
        resolved["directional"]["beta_t"] = directional.get("beta_t")
        resolved["directional"]["beta_r"] = directional.get("beta_r")
        resolved["directional"]["ht_accum_enable"] = bool(directional.get("ht_accum_enable", False))
        resolved["directional"]["ht_accum_alpha"] = directional.get("ht_accum_alpha")
        resolved["directional"]["launch_args"] = _fastlio_directional_args(
            exp_dir,
            directional=bool(directional.get("enable", False)),
            beta_t=directional.get("beta_t"),
...
def cmd_run_online(args):
    ...
    try:
        ...
        write_resolved_snapshot(
            ...
            directional={"enable": bool(getattr(args, "directional", False)),
        ...
        fastlio_cmd = ["ros2", "launch", "fast_lio", "mapping.launch.py",
                       ...
                       *_fastlio_directional_args(exp_dir,
                                                  getattr(args, "directional", False),
...
def main():
    ...
    def _add_directional_arg(sp):
        sp.add_argument("--directional", action="store_true",
                        help="P4: 启用方向性状态 correction 抑制 (directional.enable=true, "
                             "须 --algo degen / degeneracy.enable=true; 与 --adaptive 正式实验互斥)")
        sp.add_argument("--directional-beta-t", type=float, default=None,
                        metavar="X",
                        help="P4: 平动弱方向 correction 保留系数 (默认 launch 0.5)")
        sp.add_argument("--directional-beta-r", type=float, default=None,
                        metavar="X",
                        help="P4: 转动弱方向保留系数 (v1 恒 1, 不抑制转动)")
        sp.add_argument("--directional-ht-accum-enable", action="store_true",
                        help="P4: Ht 时间累积弱方向 (先累积 Ht 再特征分解, 防 sign ambiguity)")
        sp.add_argument("--directional-ht-accum-alpha", type=float, default=None,
                        metavar="X",
